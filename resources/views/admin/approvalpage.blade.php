...
<td> 
  <a href="{{ route('tampilkontenapprovaladmin', $data['id']) }}">Lihat isi konten</a>
  <td>{{ $data['folder'] }}</td>
     
      <td>
    @foreach ($data['usergroup_ids'] as $usergroup_id)
        <ul>  
            <li> <a href="{{ route('detailmemberapprovaladmin',$usergroup_id ) }}" class="detail-member">{{ \App\Models\UserGroup::find($usergroup_id)->nama_group }}</a></li> 
        </ul>
    @endforeach
</td>
...
<div class="modal fade" id="exampleModal{{ $data['id'] }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <form action="/updatestatusadmin/{{$data['id']}}" method="post" enctype="multipart/form-data">
      @csrf
      <div class="mb-3">
